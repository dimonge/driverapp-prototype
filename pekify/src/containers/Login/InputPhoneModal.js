...
import data from '../../ultils/Countries';
...
const dataCountries = data.map(item => {
  return {label: `${item.flag} - ${item.name}`, value: `${item.flag} - ${item.dial_code}`};
})
    ...
    render() {
        ...
        return (
            ...
            animationTyped='slide'>
                <SafeAreaView style={styles.containerSMS}>
                <KeyboardAvoidingView style={styles.containerSMS}
                behavior='height' enabled>
                    <TouchableWithoutFeedback style={styles.containerSMS}
                    ...
                    <View style={styles.containerSMS}>
                        ...
                        <View style={styles.formInputContainer}>
                            ...
                            <Text style={styles.dialCode}>{`(${this.state.dialCode}) `}</Text>
                            <TextInput
                                ref={ref => {this.textPhoneNumber = ref}}
                        ...
                        <TouchableOpacity style={styles.btnSendSMS} onPress={() => {
                            if(this.state.numberPhone !== '' && this.state.numberPhone) {
                                this.onSMSLoginPressHandle(this.state.dialCode + this.state.numberPhone);
                            } else {
                                toast(Languages.InputPhone);
                            }
                            
                        }}>
                            {loadingVerify ? <ActivityIndicator size="small" color="#ffffff" /> : <Text style={styles.textBtn}>{'Get code'}</Text>}
                        </TouchableOpacity>
                        </View>
