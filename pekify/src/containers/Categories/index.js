...
import { Images, Config, Constants, withTheme } from "@common";
import { toast, BlockTimer } from "@app/Omni";
import { Empty, LogoSpinner, SplitCategories, AdMob, ColumnCategories, SubCategories } from "@components";
...
class CategoriesScreen extends React.PureComponent {
  ...
  render() {
    ...
    if (Config.CategoriesLayout == Constants.CategoriesLayout.column) {
      return (
        <ColumnCategories
          onViewCategory={this.onRowClickHandle}
        />
      );
    }
    ...
    mainCategories = [...new Map(mainCategories.map(item => [item["id"], item])).values()];
