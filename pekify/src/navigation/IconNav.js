...
import { View, Platform, Image, TouchableOpacity, I18nManager, Animated, Text } from "react-native";
...
const NavBarLogo = props => {
  const scrollAnimation =
    props && !isEmpty(props.navigation) ? props.navigation.getParam("animatedHeader") : new Animated.Value(1);

  return <Animated.Image source={Config.LogoImage} style={[Styles.Common.logo, { opacity: scrollAnimation }]} />;
};
...
const NavBarText = props => {
  ...
  return <Animated.Text style={[Styles.Common.headerText, { opacity: scrollAnimation }]}>{props.text}</Animated.Text>;
};

// Icons for HeaderBar
const Logo = (title = "Pekify") => (
  <View>
...
const HeaderHomeRight = (navigation, item) => (
  <View style={[Styles.Common.Row, Platform.OS !== "ios" && { right: -12 }]}>
    {!Config.Layout.HideLayoutModal && (
      <NavigationBarIcon icon={Images.IconGrid} size={17} onPress={Events.openModalLayout} />
    )}
  </View>
);
