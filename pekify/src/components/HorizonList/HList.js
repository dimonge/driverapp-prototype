...
class HorizonList extends PureComponent {
  ...
  _viewAll = () => {
    const { config, onShowAll, index, list, fetchProductsByCollections, setSelectedCategory, fetchNews } = this.props;
  ...
  showProductsByCategory = config => {
    const { onShowAll, index, list, fetchProductsByCollections, setSelectedCategory } = this.props;
    const selectedCategory = find(list, category => category.id === config.category);
    setSelectedCategory(selectedCategory);
    fetchProductsByCollections(config.category, config.tag, this.page, index);
    onShowAll(config, index);
  ...
  renderHeader = () => {
    ...
    return (
      <HHeader showCategoriesScreen={showCategoriesScreen} config={config} theme={theme} viewAll={this._viewAll} />
    );
  };
  ...
  render() {
    ...
    const data = language.lang === "en" ? Config.HomeCategories : Config.HomeCategories_AR;
    switch (config.layout) {
      ...
      case Constants.Layout.BannerSlider:
        return <BannerSlider data={list} onViewPost={this.onViewProductScreen} />;

      case Constants.Layout.BannerImage:
        return (
          <BannerImage viewAll={this._viewAll} config={config} data={list} onViewPost={this.onViewProductScreen} />
        );
