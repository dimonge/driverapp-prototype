...
export default class ActionSheet extends Component {
    ...
    _setContainerLayout(defaultTitles = this.state.defaultTitles, cancelTitles = this.state.cancelTitles, maxHeight = height) {
        let cancelHeight = 0;
    ...
    _renderTitlesView = (titles) => {
        ...
        for (let i = 0, length = titles.length; i < length; i++) {
            ...
            let titleStyle = isDefault ? styles.activeTextStyle : this.defaultActionStyles[title.actionStyle];
            content.push(
                <TouchableOpacity
                    key={'title' + i}
                ...
                >
                    <Text accessible={false} style={[styles.defaultTextStyle, titleStyle]}>{title.title}</Text>
                </TouchableOpacity>
            );
            if (i != length - 1) {
                content.push(
    ...
    _renderCancelTitles = () => {
        ...
        content.push((<View key="separateLine" style={[styles.line, { height: separateHeight, backgroundColor: separateColor }]} />));
    ...
    render() {
        const { visible, fadeAnim, backgroundColor, defaultTitles, cancelTitles, separateColor, containerStyle, containerHeight } = this.state;
        ...
        return (
            <Modal
                ...
                <Animated.View
                    style={[styles.flex1, { opacity: fadeAnim, backgroundColor: backgroundColor }]}
                    onLayout={(event) => {
                        this._setContainerLayout(defaultTitles, cancelTitles, event.nativeEvent.layout.height);
                    }}
                >
                    <TouchableOpacity
                        onPress={() => { this.hide(); }}
                    ...
                    <Animated.View
                        style={[
                            ...
                            { transform: [{ translateY: fadeAnim.interpolate({ inputRange: [0, 1], outputRange: [containerHeight, 0] }) }] }
                        ]}
                    >
